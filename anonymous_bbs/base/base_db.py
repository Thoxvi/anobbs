...
from anonymous_bbs.resources import (
    get_env as gv,
    MongoConstant,
)
...
def get_mongo_db_uri():
    db = gv(MongoConstant.MONGO_DB_NAME)
    mongo_username = gv(MongoConstant.MONGO_INITDB_ROOT_USERNAME)
    mongo_passwd = gv(MongoConstant.MONGO_INITDB_ROOT_PASSWORD)
...
class BaseDb(object):
    ...
    def _check_exists(self, data_id: AnyStr) -> bool:
        return BaseDb._get_collection(self).count_documents({self.ID_KEY: data_id}) > 0
    ...
    def _update(self, data: dict) -> bool:
        try:
            ...
            if BaseDb._get_collection(self).find({self.ID_KEY: data_id}).count() > 0:
                BaseDb._get_collection(self).update_one({self.ID_KEY: data_id}, {"$set": data})
            else:
    ...
    def _delete(self, data_id: AnyStr) -> bool:
        return BaseDb._get_collection(self).delete_one({self.ID_KEY: data_id}).deleted_count == 1
    ...
    def _update_many(self, objects: List[dict]) -> bool:
        ...
        for obj in objects:
            if not isinstance(obj, dict):
                continue
            oid = obj.get(self.ID_KEY)
            if oid is None:
                continue
            operation_list.append(pymongo.UpdateOne({self.ID_KEY: oid}, {"$set": obj}))
    ...
    def __init__(self, uri: AnyStr, table_name: AnyStr):
        self._db_uri = uri
...
class BaseDbConnect(BaseDb):
    ...
    def __init__(self, uri: AnyStr, doc_name=__DOC_NAME):
        super().__init__(uri, doc_name)
